...
export function StoryCard({ post, className, skipImage = false, showCategories = false, showMetaInfo = false, showPreview = false, isBig = false, contentClassName = '' }: StoryCardProps) {
  return (
    <Card className={`group bg-transparent border-0 shadow-none transition-all duration-300 overflow-hidden rounded-xs ${isBig ? 'max-w-full bg-white h-full' : 'max-w-xs'} flex-shrink-0 ${skipImage ? '' : 'border-0 shadow-none'
      } ${className || ''}`}>
      ...
      <CardContent className={`${contentClassName || ''} ${isBig ? "" : "p-0"}`}>
        ...
        {/* Title */}
        <h2 className={`font-italic mb-2 leading-snug md:leading-tight ${showPreview ? 'line-clamp-3' : 'line-clamp-2 text-sm md:text-base lg:text-lg'} relative cursor-pointer overflow-hidden ${skipImage ? 'text-xl italic' : isBig ? 'text-3xl md:text-3xl font-bold lg:text-3xl xl:text-4xl italic' : 'font-medium text-lg lg:text-xl'
          }`}>
        ...
        {showPreview && (
          <p className="leading-tight text-sm mb-4 line-clamp-3">
        ...
        {/* Meta Info */}
        {showMetaInfo && (
          <div className={`flex items-center text-xs text-gray-500 ${skipImage ? 'gap-2 flex-wrap' : 'gap-4'
            }`}>
